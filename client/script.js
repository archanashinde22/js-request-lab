...
const mouseOverHelloButtonColor = (evt) => {
 
  evt.target.classList.add("color");
  const colorCss1 = document.querySelector(".color");
  colorCss1.style.color = "white";
  colorCss1.style.backgroundColor = "black";
}
sayHelloButton.addEventListener("mouseover", mouseOverHelloButtonColor);
...
// CODE HERE
const mouseOutHelloButtonColor = (evt) => {
  
  const colorCss = document.querySelector(".color");
  colorCss.style.color = "black";
  colorCss.style.backgroundColor = "#EFEFEF";
  //console.log(evt.target);
};
sayHelloButton.addEventListener("mouseout", mouseOutHelloButtonColor);
...
const ohMy = () => {
  ...
  axios.get("http://localhost:3000/animals").then((res) => {
    ...
    for (let i = 0; i < res.data.length; i++) {
      ...
      document.body.insertBefore(animalP, document.querySelector("form"));
      animalP.textContent = res.data[i];
    }
  });
};
